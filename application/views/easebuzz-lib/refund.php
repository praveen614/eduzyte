<?php
    ...
    *
    */
    function initiate_refund($params, $merchant_key, $salt, $env){
    ...
    *
    */
    function _refund($params, $merchant_key, $salt, $env){
        ...
        // check refund amount should be float or not 
        if(preg_match("/^([\d]+)\.([\d]?[\d])$/", $postedArray['refund_amount'])){
        ...
        if(!empty($postedArray['email'])){
            $email_validation = _emailValidation($postedArray['email']);
