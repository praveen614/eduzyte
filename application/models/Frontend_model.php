...
class Frontend_model extends CI_Model {
	...
	function delete_gallery($gallery_image_id)
	{
		$image=$this->db->get_where('frontend_gallery', array('frontend_gallery_id'=>$gallery_image_id))->row()->image;
		  if (file_exists('uploads/frontend/gallery_cover/'.$image)) 
		  {
			$delete=unlink('uploads/frontend/gallery_cover/'.$image);	  
			  if($delete)
			  {
				$image1=$this->db->get_where('frontend_gallery_image',array('frontend_gallery_id'=>$gallery_image_id))->result();
					
					foreach($image1 as $row)
					{
						if(file_exists('uploads/frontend/gallery_images/'.$row->image)) 
						  {
							 unlink('uploads/frontend/gallery_images/'.$row->image);
						  }
					}
				  
			  }
		  }
		  $this->db->from("frontend_gallery");
		  $this->db->where("frontend_gallery_id", $gallery_image_id);
		  $res=$this->db->delete("frontend_gallery");
		  if($res)
		  {
		  $this->db->from("frontend_gallery_image");
		  $this->db->where("frontend_gallery_id", $gallery_image_id);
		  $res=$this->db->delete("frontend_gallery_image");
		  }   
